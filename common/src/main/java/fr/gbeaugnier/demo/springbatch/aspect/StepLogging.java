...
public class StepLogging {
    ...
    @Before("execution(* fr.gbeaugnier.demo.springbatch.component.step.*..*(..))")
    public void logBefore(JoinPoint joinPoint) {
        log.info("--> {}", joinPoint.getSignature().getDeclaringType().getSimpleName());
    }

    @AfterReturning(pointcut = "execution(* fr.gbeaugnier.demo.springbatch.component.step.*..*(..))", returning = "result")
